...
export default function SharedOwnership() {
  ...
  const handleRemoveOwner = async (ownerId) => {
    try {
      ...
      // Update the status to 'declined' for both user_id1 and user_id2 if they are in "accepted" status
      const { error } = await supabase
        .from("shared_ownership")
        .update({ status: "removed" })
        .or(
          `user_id1.eq.${loggedInUserId},user_id2.eq.${ownerId},status.eq.accepted`
        )
        .or(
          `user_id2.eq.${loggedInUserId},user_id1.eq.${ownerId},status.eq.accepted`
        );

      if (error) throw error;

      // Update the shared owners locally by removing the owner with declined status
